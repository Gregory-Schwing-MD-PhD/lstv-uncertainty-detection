...
FIXED VERSION - Uses actual Net model architecture
"""
...
# Try to import timm for the ResNet encoder
try:
    import timm
    HAS_TIMM = True
except ImportError:
...
class Net(nn.Module):
    """The actual model architecture used for training"""
    def __init__(self, pretrained=False, cfg=None):
        ...
        if not HAS_TIMM:
            raise ImportError("timm is required for model. Install with: pip install timm")
...
def run_inference(args):
    ...
    if not checkpoint_path.exists():
        ...
        logger.error(f"Running in MOCK mode")
        logger.error(f"="*60)
    else:
        try:
            logger.info(f"Loading checkpoint: {checkpoint_path}")
            checkpoint = torch.load(checkpoint_path, map_location=device)
            logger.info(f"✓ Checkpoint file loaded")

            # Inspect checkpoint structure
            if isinstance(checkpoint, dict):
                checkpoint_keys = list(checkpoint.keys())
                logger.info(f"Checkpoint structure: {checkpoint_keys}")

                if 'state_dict' in checkpoint:
                    state_dict = checkpoint['state_dict']
                    logger.info(f"Found state_dict with {len(state_dict)} parameters")
                    
                    # Create model instance
                    logger.info("Instantiating Net model...")
                    ...
                    logger.error(f"Unknown checkpoint dict structure: {checkpoint_keys}")
                    model = None

            elif hasattr(checkpoint, 'eval') and hasattr(checkpoint, 'to'):
            ...
            logger.error(f"Exception loading checkpoint: {e}")
            import traceback
    ...
    if model is None:
        ...
        logger.warning("Generating synthetic uncertainty data")
        logger.warning("="*60)
    ...
    for study_id in iterator:
        ...
        if model is not None:
            ...
            # No model loaded - use mock data
            logger.debug("Using MOCK uncertainty data (no model loaded)")
            uncertainty_metrics = generate_mock_uncertainty()
